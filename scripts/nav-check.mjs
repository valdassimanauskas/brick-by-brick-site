...
for (const sel of [".promise", "#services", "#projects", "#process", "#about", "#reviews", "#contact"]) {
  await page.evaluate((s) => { document.getElementById("root").style.scrollBehavior = "auto"; const y = document.querySelector(s).getBoundingClientRect().top + document.getElementById("root").scrollTop; document.getElementById("root").scrollTo(0, y + 10); }, sel);
