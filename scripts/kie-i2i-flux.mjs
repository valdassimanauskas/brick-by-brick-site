/* kie.ai Flux 2 Pro image-to-image (same camera, changed state).
   Usage: node scripts/kie-i2i-flux.mjs "<prompt>" <refUrl[,refUrl2]> <out.png> [aspect=auto] [res=2K] */
import { readFileSync, writeFileSync } from "node:fs";
const [, , prompt, refs, outPath, aspect = "auto", resolution = "2K"] = process.argv;
if (!prompt || !refs || !outPath) { console.error("usage: kie-i2i-flux.mjs <prompt> <refUrl> <out.png> [aspect] [res]"); process.exit(1); }
...
const created = await (await fetch("https://api.kie.ai/api/v1/jobs/createTask", {
  ...
  body: JSON.stringify({ model: "flux-2/pro-image-to-image",
    input: { prompt, input_urls: refs.split(","), aspect_ratio: aspect, resolution, nsfw_checker: false } }),
})).json();
...
writeFileSync(outPath.replace(/\.png$/, ".url"), url);
console.log(`\nsaved ${outPath}\n${url}`);
