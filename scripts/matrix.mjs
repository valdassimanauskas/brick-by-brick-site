...
for (const [name, w, h, mobile, dsf] of devices) {
  ...
  await page.evaluate(() => { document.getElementById("root").style.scrollBehavior = "auto"; });
  ...
  const top = await page.evaluate(() => {
    ...
    return { overflowX: document.getElementById("root").scrollWidth > vw, h1In: inView(h1), btnIn: inView(btn), phaseIn: inView(phase),
  ...
  await page.evaluate(() => document.getElementById("root").scrollTo(0, 0.5 * (document.querySelector(".hero").offsetHeight - innerHeight)));
  ...
  await page.evaluate(() => document.getElementById("root").scrollTo(0, document.querySelector("#services").getBoundingClientRect().top + document.getElementById("root").scrollTop + 300));
  await new Promise((r) => setTimeout(r, 400));
  ...
  const railBox = await page.evaluate(() => { const r = document.querySelector(".rail"); if (!r || getComputedStyle(r).display === "none") return null; const b = r.getBoundingClientRect(); return { x: b.left + b.width * 0.8, y: b.top + b.height * 0.5, top: b.top + document.getElementById("root").scrollTop }; });
  if (railBox) {
    await page.evaluate((t) => document.getElementById("root").scrollTo(0, t - 120), railBox.top);
  ...
  await page.evaluate(() => document.getElementById("root").scrollTo(0, document.querySelector("#contact").getBoundingClientRect().top + document.getElementById("root").scrollTop - 60));
  await new Promise((r) => setTimeout(r, 400));
  await page.screenshot({ path: `shots/m/${name}-contact.png` });
  // link check
  await page.evaluate(() => { document.getElementById("root").scrollTo(0, 0); document.querySelector('.hero-actions a[href="#projects"]').click(); }); await new Promise((r) => setTimeout(r, 300));
